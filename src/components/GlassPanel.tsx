...
export default function GlassPanel({ title, onClose, children, location, className = '' }: GlassPanelProps) {
  return (
    ...
    >
      <div className="flex items-center justify-between px-4 py-3 shrink-0 border-b border-[var(--border)] gap-3 bg-[var(--bg-card)]">
        ...
        <button
          type="button"
          onClick={onClose}
          className="p-1.5 rounded-lg text-[var(--text-muted)] hover:text-[var(--text)] hover:bg-[var(--bg-base)] transition-colors"
          aria-label="Close"
        >
          <X className="w-5 h-5" />
        </button>
